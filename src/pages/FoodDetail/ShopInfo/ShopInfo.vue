...
<script>
  ...
  export default {
    ...
    data() {
      return {
        supportClasses: ['activity-orange', 'activity-green', 'activity-red'],
        initScroll: ''
      }
    },
    mounted() {
      // better-scroll在使用的时候，子元素的高度或是宽度一定要大于父元素的滚动才会生效。一般我们写的样式都是子元素会将父元素的内容撑开，所以下面要计算出实际的高或是宽度。
      this.$refs.picsUl.style.width = this.info.pics.length * 126 - 6 + 'px'
      this.$nextTick(() => {
        this.initScroll = new BScroll('.pic-wrapper', {
          scrollX: true
        })
      })
    },
